...
class User (AbstractUser):
    ...
    def meetings_attended(self):
        count = 0
        meetings = Meeting.objects.filter(requested =self).filter(requester=self).filter(was_meeting_accepted=True).filter(was_meeting_attended=True).exclude(was_meeting_cancelled=True).aggregate(count=Count('id'))
    ...
    def reliability(self):
        ...
        total_meetings = meetings = Meeting.objects.filter(requested =self).filter(requester=self).filter(was_meeting_accepted=True).exclude(was_meeting_cancelled=True).aggregate(count=Count('id'))
        #total requested requester meetings accepted, but not cancelled
        if total_meetings['count'] is not None:
            count = int(total_meetings['count'])
        attended = 1
        if all_meetings > 0:
            percentage_attandence = (float(attended)/float(all_meetings)) * 100
